...
const [loadingComentariosAdmin, setLoadingComentariosAdmin] = useState(false);
  ...
  const hasPriorityAssigned = (ticket) => {
    return !isPrioridadSinAsignar(ticket);
  };

  const canChangePriority = (ticket) => {
    return isPrioridadSinAsignar(ticket);
  };

  const isTicketAssigned = (ticket) => {
    return Boolean(ticket?.tecnicoId || ticket?.tecnico?.id || ticket?.tecnico?.nombre);
  };

  const isTicketCancelled = (ticket) => {
    return (ticket?.estadoTicket?.nombre || "").toUpperCase() === "ANULADO";
  };

  const canAssignTicket = (ticket) => {
    return !isTicketAssigned(ticket) && !isTicketCancelled(ticket);
  };
  ...
  const renderFechaDobleLinea = (fecha) => {
    ...
    return (
      <div style={{ lineHeight: 1.1 }}>
        ...
        <div style={{ fontSize: ".78rem", color: "#8a8a8a" }}>
          {formatHora(fecha)}
        </div>
      </div>
    );
  ...
  const getTecnicos = useCallback(async () => {
    try {
      ...
      const soloTecnicos = data.filter(
        (u) =>
          u.activo !== false &&
          (u.rol?.nombre === "TECNICO" ||
            u.Rols?.some((r) => r.nombre === "TECNICO"))
      );
  ...
  const getTickets = useCallback(async () => {
    try {
      ...
      if (filterPrioridad !== "todos") {
        params.append("prioridadId", filterPrioridad);
      }
      ...
      const sinAsignar = lista.filter(
        (t) =>
          !isTicketCancelled(t) &&
          !isTicketAssigned(t) &&
          ahora - new Date(t.fechaSolicitud) < 24 * 60 * 60 * 1000
      ).length;
  ...
  const handleAssign = async () => {
    if (!tecnicoSel || !selectedTicket || !canAssignTicket(selectedTicket)) {
      return;
    }
    ...
    try {
      setSaving(true);
      await axios.patch(
        `${URI_TICKETS}/${selectedTicket.id}/asignar-tecnico`,
        { tecnicoId: Number(tecnicoSel) },
        axiosCfg()
      );
  ...
  const openPriority = (ticket) => {
    ...
    setSelectedTicket(ticket);
    const currentId =
      ticket?.prioridadTicket?.id ?? ticket?.prioridadTicketId ?? "";
  ...
  const handleChangePriority = async () => {
    ...
    try {
      ...
      await axios.put(
        `${URI_TICKETS}/${selectedTicket.id}`,
        { prioridadTicketId: prioridadSel ? Number(prioridadSel) : null },
        axiosCfg()
      );
  ...
  const downloadPDF = () => {
    ...
    doc.text("REPORTE DE ASIGNACIÓN DE TICKETS", pageW / 2, 36, {
      align: "center",
    });
    ...
    doc.text(`Total de registros: ${filteredTickets.length}`, pageW - 12, 47, {
      align: "right",
    });
    ...
    autoTable(doc, {
      ...
      body: filteredTickets.map((t) => {
        ...
        const resolucion = calcularTiempoResolucion(
          t.fechaAsignacion,
          t.fechaResolucion
        );
        return [
      ...
      didDrawPage: (data) => {
        ...
        doc.text(
          `Ministerio de Salud Pública y Asistencia Social — Página ${data.pageNumber} de ${pageCount}`,
          pageW / 2,
          pageH - 8,
          { align: "center" }
        );
  ...
  const downloadExcel = () => {
    ...
    const data = filteredTickets.map((t) => {
      ...
      return {
        ...
        "Tiempo Resolución": resolucion
          ? resolucion.texto.replace("⏱ ", "")
          : "—",
      };
    });
    ...
    XLSX.writeFile(
      wb,
      `tickets_asignacion_${new Date().toISOString().slice(0, 10)}.xlsx`
    );
  };
        ...
        <tr key={ticket.id}>
          <td>
            <div className="d-flex align-items-center gap-2 flex-wrap">
              ...
              {!anulado &&
                !yaAsignado &&
                new Date() - new Date(ticket.fechaSolicitud) <
                  24 * 60 * 60 * 1000 && (
                  <span
                    style={{
                      display: "inline-block",
          ...
          <td style={{ maxWidth: 220 }}>
            <div style={{ fontSize: ".88rem", fontWeight: 800 }}>
              {ticket.tipoTicket?.nombre ?? "—"}
            </div>
            <small className="text-muted">
              {ticket.descripcion?.substring(0, 55)}
              {ticket.descripcion?.length > 55 ? "…" : ""}
            </small>
          </td>
          ...
          <td>
            ...
            <small className="text-muted">
              {ticket.departamento?.abreviatura ??
          ...
          <td style={{ whiteSpace: "nowrap" }}>
            {renderFechaDobleLinea(ticket.fechaSolicitud)}
          </td>

          <td>
            {(() => {
              const r = calcularTiempoEspera(
      ...
      {showToast && (
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            ...
            <div
              style={{ fontSize: "0.78rem", color: "#888", marginTop: 2 }}
            >
        ...
        <div className="asign-shell">
          <div className="asign-top">
            ...
            <div style={{ fontSize: ".82rem", color: "#888", marginTop: 2 }}>
              {filteredTickets.length} de {totalBackend} ticket
              {totalBackend !== 1 ? "s" : ""}
            </div>
            ...
            <div className="asign-controls">
              ...
              <div className="d-flex gap-2 flex-wrap">
                <Button
                  onClick={getTickets}
                  className="asign-btn-reload"
                  disabled={loading}
                >
                  {loading ? <Spinner size="sm" /> : "↻ Actualizar"}
                </Button>
                <Button onClick={downloadPDF} className="asign-btn-pdf">
                  PDF
                </Button>
                <Button onClick={downloadExcel} className="asign-btn-success">
                  Excel
                </Button>
              </div>
            </div>
            ...
            <div className="table-responsive">
              <table className="table table-hover align-middle mb-0">
                <thead className="asign-thead">
                  <tr>
                    <th
                      onClick={() => sortTickets("noSolicitud")}
                      style={{ cursor: "pointer", whiteSpace: "nowrap" }}
                    >
                      No. Solicitud{" "}
                      {sortField === "noSolicitud" ? (
                        <span className="ms-1">
                          {sortOrder === "asc" ? "↑" : "↓"}
                        </span>
                      ) : null}
                    ...
                    <th>Estado</th>
                    <th
                      onClick={() => sortTickets("solicitanteId")}
                      style={{ cursor: "pointer" }}
                    >
                      Solicitante{" "}
                      {sortField === "solicitanteId" ? (
                        <span className="ms-1">
                          {sortOrder === "asc" ? "↑" : "↓"}
                        </span>
                      ) : null}
                    ...
                    <th
                      onClick={() => sortTickets("fechaSolicitud")}
                      style={{ cursor: "pointer", whiteSpace: "nowrap" }}
                    >
                      Fecha{" "}
    ...
    <Button variant="outline-danger" style={{ borderRadius: 12, fontWeight: 900 }}
          ...
          {selectedTicket && (
            ...
            >
              <div style={{ fontWeight: 800, fontSize: ".9rem" }}>
                {selectedTicket.noSolicitud ?? `#${selectedTicket.id}`}
              </div>
              <div style={{ fontSize: ".85rem", color: "#555", marginTop: 2 }}>
                {selectedTicket.tipoTicket?.nombre ?? ""}
              </div>
              {selectedTicket.descripcion && (
                <div
                  style={{ fontSize: ".82rem", color: "#888", marginTop: 4 }}
                >
                  {selectedTicket.descripcion.substring(0, 120)}
                  {selectedTicket.descripcion.length > 120 ? "…" : ""}
                </div>
              )}
            </div>
          )}
          ...
          <Form.Group>
            ...
            {loadingTecnicos ? (
              <div className="text-center py-2">
                <Spinner size="sm" className="me-2" /> Cargando técnicos...
              </div>
            ) : tecnicos.length === 0 ? (
              <div className="alert alert-warning" style={{ fontSize: ".88rem" }}>
                No se encontraron técnicos activos. Verifique que existan
                usuarios con rol <strong>TECNICO</strong>.
              </div>
            ) : (
              <Form.Select
                value={tecnicoSel}
      ...
      <Modal show={showPriority} onHide={() => setShowPriority(false)} centered>
        ...
        <Modal.Body className="p-4">
          ...
          <Form.Group>
            ...
            {loadingPrioridades ? (
              <div className="text-center py-2">
                <Spinner size="sm" className="me-2" /> Cargando prioridades...
              </div>
            ) : (
              <Form.Select
                value={prioridadSel}
        ...
        <Modal.Footer>
          ...
          <Button
            ...
            disabled={savingPriority || loadingPrioridades || !prioridadSel}
      ...
      <style>{`
        ...
        .btn-accion:hover:not(:disabled) {
          ...
          background: rgba(95,125,156,0.10) !important;
        }
        ...
        .btn-accion.ok:hover:not(:disabled) {
